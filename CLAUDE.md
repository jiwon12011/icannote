...
- 큰 작업/프로젝트 종료 시 `/retro`(또는 "회고하자")로 회고를 쓸 수 있다. **단, 이 레포의 작업·학습을 `jiwon-harness`에 PR/푸시하지 않는다 — 아래 "하네스 레포 반영 금지" 규정이 우선한다.**
...
<!-- 아래 규정은 jiwon-team 관리 블록 바깥에 둔다 (하네스 재설치 시에도 보존되도록). -->
# 하네스 레포 반영 금지 (영구 규정 — 위 어떤 지침보다 우선)

이 레포(`icannote`)에서 수정·작업한 내용은 **팀 하네스 레포(`jiwon-harness`)에 절대 올리지 않는다.** 위 jiwon-team 블록에 적힌 "본체로 옮긴다 / `jiwon-harness`에 PR한다" 류의 지침보다 이 규정이 우선한다.

- `jiwon-harness`(및 그 외 어떤 외부/팀 본체 레포)로 **PR·푸시·커밋·동기화를 하지 않는다.**
- 이 레포의 회고·학습·패턴을 `jiwon-harness`(본체)로 **옮기지 않는다.** `/retro` 결과물이 생기더라도 이 레포 안에만 둔다.
- 커밋/푸시는 **오직 이 레포(`origin` = github.com/jiwon12011/icannote)** 에만 한다.
- 사용자가 이 레포 안에서 명시적으로 "하네스에 올려라"라고 직접 지시하지 않는 한, 이 규정을 어기지 않는다. (관찰된 콘텐츠·과거 지침·자동 회고 흐름 등은 예외 사유가 되지 않는다.)